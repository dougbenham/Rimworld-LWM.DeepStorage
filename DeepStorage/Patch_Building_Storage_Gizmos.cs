...
namespace LWM.DeepStorage
{
    ...
    public static class Patch_Building_Storage_Gizmos
    {
        ...
        static bool Prepare(Harmony instance)
        {
            try
            {
                var method = typeof(RimWorld.Building_Storage).GetNestedType("<GetGizmos>d__54", AccessTools.all)
                    .GetMethod("MoveNext", AccessTools.all);

                if (method == null)
                {
                    Log.Warning(
                        "LWM.DeepStorage: Transpiler could not find \"<GetGizmos>d__54\" :( -> skip Gizmos patch");
                    return false;
                }
            }
            catch (Exception e)
            {
                Log.Warning(
                    "LWM.DeepStorage: Transpiler could not find \"<GetGizmos>d__54\" :( -> skip Gizmos patch: " + e);
                return false;
            }
